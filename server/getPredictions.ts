    ...
    reader
        ...
        .on('data', (row: any) => {
            if (row.date) {
                ...
                const socialDistancePlusMask = parseInt(row["Masking + Social Diatancing"])
                const strictSocialDistancePlusMask = parseInt(row["Mandatory Masking + Strict Social Diatancing"])
